(function() {
  ...
  var api_url = 'https://kinogo.ec/api/';
  ...
  function kinogo(component, _object) {
      ...
      ping_auth = setInterval(function() {
        network.silent(Lampa.Utils.addUrlComponent(api_url + 'user_profile', dev_token + user_token), function(json) {
          if (json && json.user_data) {
            Lampa.Modal.close();
            clearInterval(ping_auth);
            Lampa.Storage.set("kinogo_token", user_token);
    ...
    this.searchByTitle = function(_object, query) {
      ...
      url = Lampa.Utils.addUrlComponent(url, dev_token + kinogo_token);
      network.clear();
    ...
    this.find = function(kinogo_id) {
      var url = proxy_url + api_url;

      end_search(kinogo_id);

      function end_search(kinogo_id) {
        network.clear();
        network.timeout(10000);
        network.silent(url + 'post/' + kinogo_id + '?' + dev_token + kinogo_token, function(found) {
      ...
      if (pl_links.playlist && Object.keys(pl_links.playlist).length > 0) {
          ...
          for (var voice in episode) {
            ...
            for (var ID in episode_voice) {
              ...
              var quality_eps = file_episod.qualities.filter(function(qualitys) {
                return qualitys <= window.kinogo.max_qualitie;
              });
        ...
        for (var _ID in pl_links.movie) {
          ...
          if (_quality_eps) _quality_eps = _quality_eps[1].split(',').filter(function(quality_) {
            return quality_ <= window.kinogo.max_qualitie;
          });
  ...
  function startPlugin() {
    ...
    Lampa.Lang.add({
      ...
      modal_text: {
        ru: 'Введите код на странице https://kinogo.ec/activate',
        uk: 'Введіть код на сторінці https://kinogo.ec/activate',
        en: 'Enter the code on the page https://kinogo.ec/activate',
        zh: '在页面 https://kinogo.ec/activate 输入代码'
      },
      modal_wait: {
        ru: 'Ожидаем код',
        uk: 'Очікуємо код',
    ...
    function checkToken(token) {
      ...
      network.silent(Lampa.Utils.addUrlComponent(api_url + 'user_profile', dev_token + token), function(json) {
        if (json) {
          if (json.user_data) {
            if (json.user_data.is_pro) window.kinogo.max_qualitie = 1080;
            if (json.user_data.is_pro_plus) window.kinogo.max_qualitie = 2160;
          } else {
            Lampa.Storage.set('kinogo_token', '');
            kinogo_token = '';
          }
        }
      }, function(a, c) {
    ...
    var token = Lampa.Storage.get('kinogo_token', '');
    if (token)
      checkToken(token);
