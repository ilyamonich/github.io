(function() {
  ...
  var kinogo_url = 'https://kinogo.ec/';
  ...
  function kinogo(component, _object) {
    ...
    function parseSearchResults(html) {
      ...
      items.forEach(function(item) {
        var titleElem = item.querySelector('.short-title h2 a');
        var linkElem = titleElem;
        var imageElem = item.querySelector('.short-img img');
        var infoElem = item.querySelector('.short-content');
        
        if (titleElem && linkElem) {
          var yearMatch = infoElem ? infoElem.textContent.match(/(\d{4})/) : null;
          
          results.push({
            id: linkElem.href.split('-').pop().replace('.html', ''),
    ...
    function parseMoviePage(html) {
      ...
      // Поиск ссылок на видео в плеерах
      var players = doc.querySelectorAll('.player');
      ...
      network.native(search_url, {
        ...
        try {
          ...
          var card = cards.find(function(c) {
            return (!c.year || c.year == year) && normalizeString(c.title) == normalizeString(orig);
          });
    ...
    this.find = function(kinogo_id, kinogo_url) {
      var url = kinogo_url || (kinogo_url + kinogo_id + '.html');
      ...
      network.native(url, function(html) {
        try {
          ...
          if (found && (found.player_links.movie.length > 0 || Object.keys(found.player_links.playlist).length > 0)) {
            success(found);
            component.loading(false);
          } else {
            component.doesNotAnswer();
    ...
    function extractData(data) {
      ...
      if (data.player_links.playlist && Object.keys(data.player_links.playlist).length > 0) {
        // Обработка сериалов (если есть плейлисты)
        for (var season in data.player_links.playlist) {
          var episode = data.player_links.playlist[season];
          for (var voice in episode) {
            var episode_voice = episode[voice];
            var transl_id = Object.keys(extract).length + 1;
            ...
            episode_voice.forEach(function(file_episod, index) {
              extract[transl_id].json.push({
                id: season + '_' + (index + 1),
                comment: 'Сезон ' + season + ' Эпизод ' + (index + 1),
                file: file_episod.link,
                episode: index + 1,
      ...
      if (translat) {
        if (element.season) {
          ...
          for (var i in translat.json) {
            ...
            if (elem.id === element.season + '_' + element.episode) {
              file = elem.file;
              break;
            }
          }
    ...
    function filter() {
      ...
      // Для Kinogo обычно только один вариант перевода
      filter_items.voice.push('Оригинал');
    ...
    function filtred() {
      ...
      if (Object.keys(results.player_links.playlist).length) {
        ...
        for (var transl in extract) {
          ...
          element.json.forEach(function(media) {
            filtred.push({
              episode: media.episode,
              season: media.season,
              title: media.comment,
              quality: '1080p',
              translation: parseInt(transl),
              voice_name: element.translation || 'Оригинал',
              info: element.translation || 'Оригинал'
            });
          });
        }
  ...
  function startPlugin() {
    ...
    // Нужна заглушка для инициализации
    Lampa.Component.add('online_kinogo', component);
    resetTemplates();
