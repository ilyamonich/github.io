(function() {
  ...
  var plugin_name = 'kinokong';
  var api_url = 'https://kinokong.day/';
  var search_url = api_url + 'index.php?do=search';
  var modalopen = false;

  function kinokong(component, _object) {
    ...
    function parseSearchResults(html) {
      ...
      var items = doc.querySelectorAll('.th-item');
      items.forEach(function(item) {
        var titleElem = item.querySelector('.th-title');
        var linkElem = item.querySelector('a');
        var yearElem = item.querySelector('.th-year');
        
        if (titleElem && linkElem) {
          var title = titleElem.textContent.trim();
          var href = linkElem.href;
          var year = yearElem ? parseInt(yearElem.textContent) : null;
    ...
    function parseMoviePage(html, filmId) {
      var result = {
        id: filmId,
        player_links: {
          movie: [],
      ...
      var videoPlayers = doc.querySelectorAll('.video-player, iframe[src*="youtube"], iframe[src*="vimeo"]');
      var translationId = 0;
      
      videoPlayers.forEach(function(player, index) {
        translationId++;
        var src = player.getAttribute('data-src') || player.src;
        
        if (src && (src.includes('youtube') || src.includes('vimeo') || src.includes('.mp4'))) {
          result.player_links.movie.push({
            translation: 'Перевод ' + translationId,
            link: src,
            quality: 720
          });
        }
      ...
      // Парсим ссылки на сериалы
      var seasonBlocks = doc.querySelectorAll('.season-block');
      seasonBlocks.forEach(function(seasonBlock, seasonIndex) {
        var seasonNum = seasonIndex + 1;
        result.player_links.playlist[seasonNum] = {};
        
        var episodeBlocks = seasonBlock.querySelectorAll('.episode-block');
        episodeBlocks.forEach(function(episodeBlock, episodeIndex) {
          var episodeNum = episodeIndex + 1;
          var translationId = 1; // По умолчанию один перевод
          
          if (!result.player_links.playlist[seasonNum][translationId]) {
            result.player_links.playlist[seasonNum][translationId] = {};
          }
          
          var videoLinks = episodeBlock.querySelectorAll('a[data-link]');
          videoLinks.forEach(function(link, linkIndex) {
            var videoSrc = link.getAttribute('data-link');
            if (videoSrc) {
              result.player_links.playlist[seasonNum][translationId][episodeNum] = {
                link: videoSrc,
    ...
    this.searchByTitle = function(_object, query) {
      ...
      network.native(search_url, function(html) {
        ...
        var cards = searchResults.filter(function(c) {
          return !c.year || (c.year > year - 2 && c.year < year + 2);
        });
        
        var card = cards.find(function(c) {
          return normalizeString(c.title).includes(normalizeString(orig)) || 
                 normalizeString(c.title).includes(normalizeString(object.movie.title || object.movie.name));
        });
    ...
    this.find = function(filmId) {
      var url = api_url + filmId + '.html';
      
      network.clear();
      network.timeout(15000);
      network.silent(url, function(html) {
        if (html && html.length > 1000) {
          var movieData = parseMoviePage(html, filmId);
          success(movieData);
          component.loading(false);
        } else {
          component.doesNotAnswer();
    ...
    function extractData(data) {
      ...
      if (pl_links.playlist && Object.keys(pl_links.playlist).length > 0) {
          ...
          for (var voice in episode) {
            ...
            for (var ID in episode_voice) {
              ...
              var quality_eps = file_episod.qualities.filter(function(qualitys) {
                return qualitys <= window.kinokong.max_qualitie;
              });
              ...
              if (stream_url) {
                items.push({
                  id: seas_num + '_' + ID,
                  comment: ID + ' ' + Lampa.Lang.translate('torrent_serial_episode'),
                  file: stream_url,
                  episode: parseInt(ID),
                  season: seas_num,
                  quality: max_quality,
        ...
        for (var _ID in pl_links.movie) {
          ...
          var _quality_eps = _file_episod.qualities || [360, 480, 720, 1080];
          _quality_eps = _quality_eps.filter(function(quality_) {
            return quality_ <= window.kinokong.max_qualitie;
          });

          var _max_quality = Math.max.apply(null, _quality_eps);

          extract[_transl_id] = {
            file: _file_episod.link,
    ...
    function getFile(element, max_quality) {
      ...
      if (file) {
        ...
        // Для kinokong обычно одна ссылка, без вариантов качества
      ...
      for (var Id in results.player_links.playlist) {
        ...
        for (var voic in season) {
          ...
          if (filter_items.voice.indexOf(voic) == -1) {
            filter_items.voice.push('Перевод ' + d);
  ...
  function startPlugin() {
    window.online_kinokong = true;
    var manifest = {
      type: 'video',
      version: '1.0.0',
      name: 'Онлайн - Kinokong',
      description: 'Плагин для просмотра онлайн сериалов и фильмов с Kinokong.day',
      component: 'online_kinokong',
      ...
      onContextLauch: function onContextLauch(object) {
        resetTemplates();
        Lampa.Component.add('online_kinokong', component);
        Lampa.Activity.push({
          url: '',
    ...
    if (Lampa.Manifest.app_digital >= 177) {
      Lampa.Storage.sync('online_choice_kinokong', 'object_object');
    }
  }

  if (!window.online_kinokong && Lampa.Manifest.app_digital >= 155) startPlugin();
