(function() {
  ...
  function component(object) {
    ...
    this.startSource = function(json) {
      return new Promise(function(resolve, reject) {
        // Принудительно добавляем Kinogo.ec если его нет в списке
        var hasKinogo = json.some(function(j) {
          return balanserName(j) === 'kinogo';
        });
        
        if (!hasKinogo) {
          json.push({
            name: 'Kinogo.ec',
            url: Defined.localhost + 'lite/kinogo',
            balanser: 'kinogo',
            show: true
          });
        }
      ...
      return new Promise(function(resolve, reject) {
        ...
        var fin = function fin(call) {
          ...
          network.silent(account(url), function(json) {
            ...
            // Добавляем Kinogo.ec если его нет
            if (!sources['kinogo']) {
              sources['kinogo'] = {
                url: Defined.localhost + 'lite/kinogo',
                name: 'Kinogo.ec',
            ...
            if (life_wait_times > 15 || json.ready) {
              filter.render().find('.kinobaza-balanser-loader').remove();
              gou(json, true);
      ...
      return new Promise(function(resolve, reject) {
        ...
        network.silent(account(url), function(json) {
          ...
          if (json.life) {
            ...
            filter.render().find('.filter--sort').append('<span class="kinobaza-balanser-loader" style="width: 1.2em; height: 1.2em; margin-top: 0; background: url(./img/loader.svg) no-repeat 50% 50%; background-size: contain; margin-left: 0.5em"></span>');
            _this4.lifeSource().then(_this4.startSource).then(resolve)["catch"](reject);
          } else {
            _this4.startSource(json).then(resolve)["catch"](reject);
          }
        }, function() {
          // Если сервер не отвечает, создаем ручной список с Kinogo.ec
          var manualSources = [{
            name: 'Kinogo.ec',
            url: Defined.localhost + 'lite/kinogo',
            balanser: 'kinogo',
          ...
          _this4.startSource(manualSources).then(resolve)["catch"](reject);
        });
      });
    };
      ...
      this.draw(videos, {
        onEnter: function onEnter(item, html) {
          _this5.getFileUrl(item, function(json, json_call) {
            ...
            } else Lampa.Noty.show(Lampa.Lang.translate('kinobaza_nolink'));
          }, true);
        },
    ...
    this.similars = function(json) {
      ...
      json.forEach(function(elem) {
        ...
        var item = Lampa.Template.get('kinobaza_prestige_folder', elem);
		if (elem.img) {
    ...
    this.updateWatched = function() {
      ...
      } else body.append('<span>' + Lampa.Lang.translate('kinobaza_no_watch_history') + '</span>');
    };
    this.draw = function(items) {
      ...
      scroll.clear();
      if(!object.balanser)scroll.append(Lampa.Template.get('kinobaza_prestige_watched', {}));
      this.updateWatched();
      this.getEpisodes(items[0].season, function(episodes) {
        ...
        items.forEach(function(element, index) {
          ...
          if (episode) {
            ...
            if (element.info.length < 30 && episode.vote_average) info.push(Lampa.Template.get('kinobaza_prestige_rate', {
        ...
        if (serial && episodes.length > items.length && !params.similars) {
          ...
          left.forEach(function(episode) {
            var info = [];
            if (episode.vote_average) info.push(Lampa.Template.get('kinobaza_prestige_rate', {
              rate: parseFloat(episode.vote_average + '').toFixed(1)
            }, true));
            ...
            var html = Lampa.Template.get('kinobaza_prestige_full', {
              time: Lampa.Utils.secondsToTime((episode ? episode.runtime : object.movie.runtime) * 60, true),
    ...
    this.contextMenu = function(params) {
      params.html.on('hover:long', function() {
        function show(extra) {
          ...
          menu.push({
            title: Lampa.Lang.translate('kinobaza_video'),
            separator: true
          });
          menu.push({
          ...
          if (window.kinobaza_online_context_menu)
            window.kinobaza_online_context_menu.push(menu, extra, params);
          menu.push({
          ...
          if (Lampa.Account.logged() && params.element && typeof params.element.season !== 'undefined' && params.element.translate_voice) {
            menu.push({
              title: Lampa.Lang.translate('kinobaza_voice_subscribe'),
              subscribe: true
            });
          }
          menu.push({
            title: Lampa.Lang.translate('kinobaza_clear_all_marks'),
            clearallmark: true
          });
          menu.push({
            title: Lampa.Lang.translate('kinobaza_clear_all_timecodes'),
            timeclearall: true
          });
          Lampa.Select.show({
            ...
            onSelect: function onSelect(a) {
              ...
              if (window.kinobaza_online_context_menu)
                window.kinobaza_online_context_menu.onSelect(a, params);
              Lampa.Controller.toggle(enabled);
              if (a.player) {
              ...
              if (a.subscribe) {
                Lampa.Account.subscribeToTranslation({
                  ...
                  Lampa.Noty.show(Lampa.Lang.translate('kinobaza_voice_success'));
                }, function() {
                  Lampa.Noty.show(Lampa.Lang.translate('kinobaza_voice_error'));
                });
              }
            }
          });
    ...
    this.noConnectToServer = function(er) {
      ...
      html.find('.online-empty__time').text(er && er.accsdb ? er.msg : Lampa.Lang.translate('kinobaza_does_not_answer_text').replace('{balanser}', sources[balanser] ? sources[balanser].name : balanser));
      scroll.clear();
      scroll.append(html);
      this.loading(false);
    };
  ...
  function addKinogoSearch() {
    var source = {
      ...
      search: function(params, oncomplite) {
        ...
        network.silent(account(Defined.localhost + 'lite/kinogo?title=' + encodeURIComponent(params.query)), function(json) {
          if (json && json.rch) {
            rchRun(json, function() {
              network.silent(account(Defined.localhost + 'lite/kinogo?title=' + encodeURIComponent(params.query)), function(links) {
  ...
  function startPlugin() {
    ...
    console.log('Кинобаза: Инициализация плагина с поддержкой Kinogo.ec');

    var manifst = {
      type: 'video',
      version: '1.5.8',
      name: 'Кинобаза',
      description: 'Плагин для просмотра онлайн сериалов и фильмов с поддержкой Kinogo.ec',
      component: 'kinobaza',
    ...
    Lampa.Lang.add({
      ...
      kinobaza_voice_subscribe: { ru: 'Подписаться на перевод', uk: 'Підписатися на переклад', en: 'Subscribe to translation', zh: '订阅翻译' },
      kinobaza_voice_success: { ru: 'Вы успешно подписались', uk: 'Ви успішно підписалися', en: 'You have successfully subscribed', zh: '您已成功订阅' },
      kinobaza_voice_error: { ru: 'Возникла ошибка', uk: 'Виникла помилка', en: 'An error has occurred', zh: '发生了错误' },
      kinobaza_clear_all_marks: { ru: 'Очистить все метки', uk: 'Очистити всі мітки', en: 'Clear all labels', zh: '清除所有标签' },
      kinobaza_clear_all_timecodes: { ru: 'Очистить все тайм-коды', uk: 'Очистити всі тайм-коди', en: 'Clear all timecodes', zh: '清除所有时间代码' },
      kinobaza_change_balanser: { ru: 'Изменить источник', uk: 'Змінити джерело', en: 'Change source', zh: '更改来源' },
      kinobaza_balanser_dont_work: { ru: 'Источник не отвечает', uk: 'Джерело не відповідає', en: 'Source is not responding', zh: '来源无响应' },
      kinobaza_balanser_timeout: { ru: 'Источник будет переключен автоматически через <span class="timeout">10</span> секунд.', uk: 'Джерело буде автоматично переключено через <span class="timeout">10</span> секунд.', en: 'The source will be switched automatically after <span class="timeout">10</span> seconds.', zh: '来源将在<span class="timeout">10</span>秒内自动切换。' },
      kinobaza_does_not_answer_text: { ru: 'Поиск не дал результатов', uk: 'Пошук не дав результатів', en: 'Search did not return any results', zh: '搜索未返回任何结果' }
    });
    ...
    console.log('Кинобаза: Плагин успешно инициализирован с поддержкой Kinogo.ec');
  }
